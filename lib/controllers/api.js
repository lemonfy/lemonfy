...
var crypto = require('../helpers/crypto_util'),
    ...
    validationUtil = require('../helpers/validation_util'),
...
exports.companyNames = function(req, res, next) {
  ...
  Company.find({}, {id: 1, n: 1}, {sort: {n: 1}}, function (err, companies) {
    if (!err) {
  ...
  Company.findOne({$or: [{d: company.d}, {n: company.n}]}, {id: 1}, 
    function(err, sameCompany) {
      if (err) {
        ...
        var msg = 'Company already exists';
        var e = new Error(msg);
        e.code = 400;
        e.msg = msg;
...
exports.ratingSubmit = function(req, res, next) {
  ...
  if (!attrs || !isValidString(attrs.cid) || !isValidString(attrs.email) || !validationUtil.checkScore(attrs.s)) {
  ...
  Company.findOne({_id: attrs.cid}, {_id: 1, d: 1}, function(err, company) {
    ...
    var email = attrs.email.trim().toLowerCase() + '@' + company.d;
    var rating = {
      cid: company._id,
      h: crypto.hash(email),
      v: false,
